...
>
	...
	{#if lightboxVisibility}
		<div
			...
			style:--bg-color-lightbox={aboutme?.lightboxBgColor
				?.hex ?? "#fff"}
			style:--primary-color-lightbox={aboutme
				?.lightboxPrimaryColor?.hex ?? "#000"}
		>
					...
					<div class="buttons">
						...
						>
							...
							>
								...
								<path
									d="M186 741L186 18"
									stroke="var(--primary-color-lightbox)"
									stroke-width="30"
...
<style lang="postcss">
	...
	* {
	...
	.email-lightbox-container {
		...
		height: 100vh;
		width: 100%;
		background-color: var(--bg-color-lightbox);
		z-index: 1000;
	}
	...
	input {
		padding-block: 4px;
		border-bottom: 1px solid var(--primary-color-lightbox);
	}
	textarea {
		margin-top: 8px;
		padding: 8px;
		border: 1px solid var(--primary-color-lightbox);
		border-radius: 4px;
		white-space: prewrap;
	}
	a,
